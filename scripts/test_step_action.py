...
def test_step_action():
    ...
    # Grid
    chi_values = np.linspace(1.01, 20.0, 50).tolist()
    ml_values = list(range(-10, 11))
    sigma3_values = [1, -1]
    
    orc = Orchestrator(device="cpu")
    
    print("Computing Numerical EA for Step Function...")
    # Disable UV sub for a moment to see divergence
