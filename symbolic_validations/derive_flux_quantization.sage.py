...
# This file was *autogenerated* from the file symbolic_validations/derive_flux_quantization.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_5 = Integer(5); _sage_const_6 = Integer(6); _sage_const_0p5 = RealNumber('0.5')# SageMath Script: Derive and Validate Flux Quantization Check
from sage.all import *

# Define symbols
rho, F_bar, chi, m, lambd = var('rho F_bar chi m lambd')
assume(chi > m, rho > _sage_const_0 , lambd > _sage_const_0 )
...
k = sqrt(chi**_sage_const_2  - m**_sage_const_2 )
ml = var('ml')

def integrand(ml_val, F_val):
    return bessel_J(ml_val - F_val, k * rho) * bessel_Y(ml_val - F_val, k * rho) -            bessel_J(ml_val, k * rho) * bessel_Y(ml_val, k * rho)
...
ml_range = range(-_sage_const_5 , _sage_const_6 )

sum_int = sum([integrand(m_i, F_int) for m_i in ml_range])
print(f"Sum of integrand for F={F_int} over ml in {-_sage_const_5 }..{_sage_const_5 }: {sum_int.simplify_full()}")

# Test for non-integer flux F=0.5
F_nonint = _sage_const_0p5 
sum_int_non = sum([integrand(m_i, F_nonint) for m_i in ml_range])
print(f"Sum of integrand for F={F_nonint} over ml in {-_sage_const_5 }..{_sage_const_5 }: {sum_int_non.simplify_full()}")
